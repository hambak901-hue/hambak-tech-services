...
const API = {
  ...
  // Linked Transaction Gateway Methods
  transactions: {
    initializeDeposit: async (amount) => {
      const response = await fetch(`${LIVE_API_URL}/transactions/deposit/initialize`, {
        method: "POST",
        headers: getAuthHeaders(),
    ...
    verifyDeposit: async (reference) => {
      const response = await fetch(`${LIVE_API_URL}/transactions/deposit/verify`, {
        method: "POST",
        headers: getAuthHeaders(),
...
// Export to window execution memory spaces for script components
window.API = API;
